...
def computeExtrinsic(img_path, mtx, dist, dX, dY):
    ...
    # Ensure there are exactly 4 points captured
    if len(X_CAPT) != 4 or len(Y_CAPT) != 4:
        raise ValueError("Four points are required, but got different number.")

    img_points = np.vstack((X_CAPT, Y_CAPT)).T.reshape(-1, 1, 2)
    obj_points = np.array([[0, dX, dX, 0], [0, 0, dY, dY], [0, 0, 0, 0]]).T.reshape(-1, 1, 3)

    # Debug: Print shapes and values of obj_points and img_points
    print("Image Points Shape:", img_points.shape, "Object Points Shape:", obj_points.shape)
    print("Image Points:", img_points, "Object Points:", obj_points)
    ...
    # Using solvePnP instead of solvePnPRansac for testing
    _, rvec, tvec = cv2.solvePnP(obj_points, img_points, mtx, dist)
    rmat = cv2.Rodrigues(rvec)[0]

    # Displaying the extrinsic parameters
